...
class MyDimens {
  ...
  ///获取适配后的字体尺寸
  static double getSp(double sp){
    return ScreenUtil.getInstance().getSp(sp);
  }

  /// 横向间隔
  static Widget width({@required double width}) {
    return SizedBox(
      width: MyDimens.getWidth(width),
    );
  }
  ...
  static Widget height({@required double height}) {
    return SizedBox(
      height: MyDimens.getHeight(height),
    );
  }


  static BorderSide borderSide({double width,Color color}){
